...
Her ürün: `text` (ad), `done`, `order`, `createdAt`; isteğe bağlı `amount` (sayı) ve `unit` (`kg`, `adet`, `g`, `L`, `paket`). Miktar yoksa sadece ürün adı yeterlidir (eski kayıtlar uyumludur). Liste ekranında ürün **+** ile sheet açılır; birim isteğe bağlıdır, miktar yalnızca elle girilir.
